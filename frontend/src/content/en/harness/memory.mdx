...
The default tier-aware injection mode reads full L2 files while retaining
compact L0/L1 summaries. Explicit searches default to full-content reads so a
successful match carries the evidence needed to answer, rather than only its
overview. The legacy `retrieval.content_mode` setting remains supported and
sets both behaviors.
